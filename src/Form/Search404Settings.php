...
class Search404Settings extends ConfigFormBase
{
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        $form['advanced']['search404_base_name'] = array(
            '#type' => 'checkbox',
            '#title' => t('Use base name for searching'),
            '#description' => t('This feature will conduct a search only on the last part of the url. For example, if the url http://www.example.com/example/test/this-is-a-test then it will only search for "this is a test" only not "example or test" '),
            '#default_value' => \Drupal::config('search404.settings')->get('search404_base_name'),
        );
        $form['advanced']['search404_add_oldurl_querystring'] = array(
            '#type' => 'checkbox',
            '#title' => t('Append old url'),
            '#description' => t('This feature will append the original url to the search url for Analytics purposes '),
            '#default_value' => \Drupal::config('search404.settings')->get('search404_add_oldurl_querystring'),
        );
    ...
    public function submitForm(array &$form, FormStateInterface $form_state)
    {
        ...
        $settings->set('search404_redirect_301', $form_state->getValue('search404_redirect_301'))
            ...
            ->set('search404_base_name', $form_state->getValue('search404_base_name'))
            ->set('search404_add_oldurl_querystring', $form_state->getValue('search404_add_oldurl_querystring'));
